...
1. Open the terminal and install test dependencies.(1)
   { .annotate }
    ...
    !!! note "Linux dependency note"

        If your Linux distribution is not officially supported by Playwright (for example Debian trixie), `npx playwright install --with-deps` may fail because it tries Ubuntu-specific package names.

        Install browser runtime dependencies using your distro package manager, then run `npx playwright install chromium`.

        For Debian trixie, the following command works:

        ```shell
        sudo apt-get update && sudo apt-get install -y libdbus-1-3 libatk1.0-0 libatk-bridge2.0-0 libcups2 libdrm2 libatspi2.0-0 libxcomposite1 libxdamage1 libxfixes3 libxrandr2 libgbm1 libxkbcommon0 libasound2
        ```
